...
原因は `sort` コマンドが参照している `LC_COLLATE` の設定にありそう。
...
`sort -u` は「ソート順序が同じなら重複とみなす」という挙動をする。
現在の設定だと、ローマ数字の１から３をすべて同じものとして扱っている。結果、最初の1行だけを残して他を削除してしまう。これはローマ数字に限らず、丸数字（①）でも同様の現象が起こる。

...
// Dynamically import all images from /src/content/articles/_images
const imagesGlob = import.meta.glob<{ default: ImageMetadata }>(
  '/src/content/articles/_images/**/*.{jpeg,jpg,png,gif,webp}',
)
...
const getOptimizedImageUrl = memoize(async (srcPath: string, baseUrl: string) => {
  ...
  const rawImagePath = `/src/content/articles/${prefixRemoved}`
  ...
  for (const img of images) {
    ...
    imagePromises.push((async () => {
      try {
        ...
        // Process images from src/content/articles/_images directory
...
function generatePostUrl(post: CollectionEntry<'posts'>, baseUrl: string): string {
  ...
  return new URL(`${langPrefix}articles/${postSlug}/`, baseUrl).toString()
}
